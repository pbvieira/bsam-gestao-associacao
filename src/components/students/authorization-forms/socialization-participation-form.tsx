...
interface SocializationParticipationFormProps {
  name: string;
  rg: string | null;
  cpf: string | null;
  date?: string;
...
export function SocializationParticipationForm({
  name,
  rg,
  cpf,
  date = new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' }),
  city = 'São José - SC'
}: SocializationParticipationFormProps) {
  const handlePrint = () => {
    window.print();
  };
    ...
    <>
      ...
      <div className="min-h-screen bg-muted print:bg-white print:min-h-0 py-8 print:py-0 flex justify-center">
        <div className="print-container bg-white w-full max-w-[210mm] min-h-[297mm] p-12 shadow-lg print:shadow-none mx-4 print:mx-0">
          ...
          {/* Body Text */}
          <div className="font-serif text-sm text-black text-justify leading-relaxed mt-8">
            <p className="indent-8">
              <strong>{name}</strong> Portador do RG/CPF <strong>{formatDocument()}</strong> declaro 
              que participei do programa de socialização sendo orientado das normas e procedimentos 
              da Associação de Assistência Social e Educacional "O Bom Samaritano", e estou de acordo 
              e ciente que o não cumprimento do regimento interno pode acarretar no meu desligamento.
            </p>
          </div>

          {/* Familiar Participante Section */}
          <div className="mt-12">
            <p className="font-serif text-sm text-black mb-4">Familiar Participante:</p>
            <div className="space-y-8">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="h-8 border-b border-black" />
              ))}
            </div>
          </div>

          {/* Signature Section */}
          <div className="mt-16 text-right font-serif text-sm text-black">
            <p>{city}, {date}</p>
