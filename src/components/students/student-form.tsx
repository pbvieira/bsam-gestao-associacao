...
function StudentFormContent({ student, onSuccess, onCancel, onRefreshPhoto }: StudentFormProps) {
  ...
  const form = useForm<StudentHeaderForm>({
    ...
    defaultValues: {
      ...
      data_saida: student?.data_saida ? `${student.data_saida}T${student.hora_saida || "00:00"}` : "",
  ...
  const saveHeader = async (): Promise<{ success: boolean; newStudentId?: string }> => {
    ...
    try {
      ...
      if (data.cpf && data.cpf.length > 0) {
        ...
        let query = supabase.from("students").select("id, nome_completo").eq("cpf", data.cpf);
      ...
      if (data.rg && data.rg.length > 0) {
        let query = supabase.from("students").select("id, nome_completo").eq("rg", data.rg);
    ...
    <div className="space-y-6">
      {isCreationMode && (
        <Alert>
          ...
          <AlertDescription>
            Preencha primeiro as informações principais do aluno. Após salvar, você poderá acessar as outras abas para
        ...
        <TabsContent value="header" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              {student?.nome_completo && <p className="text-sm text-muted-foreground">{student.nome_completo}</p>}
            </CardHeader>
            <CardContent>
              <Form {...form}>
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input value={permanencia} disabled className="bg-muted" />
                      </FormControl>
                    </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="flex flex-row items-center space-x-3 space-y-0 pt-6">
                          ...
                          <FormLabel className="font-normal cursor-pointer">Não possui documentos</FormLabel>
                        </FormItem>
                      )}
                    />
          ...
          {savedStudentId && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start">
                ...
                <DropdownMenuItem
                  onClick={() => window.open(`/alunos/${savedStudentId}/integracao-voluntaria`, "_blank")}
                >
                  <FileSignature className="h-4 w-4 mr-2" />
                  Declaração de Integração Voluntária
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => window.open(`/alunos/${savedStudentId}/participacao-socializacao`, "_blank")}
                >
                  <FileSignature className="h-4 w-4 mr-2" />
                  Declaração de Participação na Socialização
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => window.open(`/alunos/${savedStudentId}/termo-responsabilidade`, "_blank")}
                >
                  <FileSignature className="h-4 w-4 mr-2" />
